<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 p-4 bg-pink-50 dark:bg-pink-950/20 rounded-lg border border-pink-200 dark:border-pink-800/40">
            ...
            <div>
                <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Presentasi Janin</label>
                <select wire:model="anc_presentasi" class="block w-full rounded-lg border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-3 py-2 text-sm text-zinc-900 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" {{ !$isEditable ? 'disabled' : '' }}>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Golongan Darah</label>
                <select wire:model="anc_golongan_darah" class="block w-full rounded-lg border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-3 py-2 text-sm text-zinc-900 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" {{ !$isEditable ? 'disabled' : '' }}>
            ...
            <div class="flex flex-col gap-2">
                <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Riwayat Sectio Caesarea</label>
                <div class="flex items-center h-10">
                    <label class="flex items-center gap-2 text-sm text-zinc-700 dark:text-zinc-300 cursor-pointer">
                        <input type="checkbox" wire:model="anc_riwayat_sc" class="rounded border-zinc-300 text-teal-600 focus:ring-teal-500" {{ !$isEditable ? 'disabled' : '' }}>
                        <span>Pernah SC sebelumnya</span>
                    </label>
                </div>
            </div>
    ...
    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm">
        ...
        <div class="p-5 bg-zinc-50 dark:bg-zinc-950/40 border border-zinc-200/60 dark:border-zinc-800/80 rounded-xl mb-6 space-y-4">
            <div class="flex items-center gap-6">
                <label class="flex items-center gap-2 text-sm text-zinc-700 dark:text-zinc-300 cursor-pointer">
                    <input type="radio" wire:model.live="presc_type" value="non-racikan" class="text-indigo-600 focus:ring-indigo-500 border-zinc-300" {{ !$isEditable ? 'disabled' : '' }}>
                    <span>Non-Racikan (Standar)</span>
                </label>
                <label class="flex items-center gap-2 text-sm text-zinc-700 dark:text-zinc-300 cursor-pointer">
                    <input type="radio" wire:model.live="presc_type" value="racikan" class="text-indigo-600 focus:ring-indigo-500 border-zinc-300" {{ !$isEditable ? 'disabled' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 border-b border-zinc-200/60 dark:border-zinc-800/80 pb-4">
                ...
                <div>
                    ...
                    <select wire:model="presc_metode_racik_id" class="block w-full rounded-lg border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-3 py-2 text-sm text-zinc-900 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" {{ !$isEditable ? 'disabled' : '' }}>
                        <option value="">Pilih Metode</option>
                        @foreach ($metodeRaciks as $m)
                        <option value="{{ $m->id }}" wire:key="metode-opt-{{ $m->id }}">{{ $m->nama_metode_racik }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                <div class="md:col-span-6 relative">
                    ...
                    <div class="relative rounded-md shadow-sm">
                        ...
                        <input type="text" wire:model.live.debounce.250ms="drugQuery" placeholder="Ketik nama obat..." class="block w-full rounded-lg border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 pl-10 pr-3 py-2 text-sm text-zinc-900 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" {{ !$isEditable ? 'disabled' : '' }}>
                    </div>
                    @if (count($drugResults) > 0)
                    <div wire:key="drug-dropdown-list-container" class="absolute z-50 left-0 right-0 mt-1 max-h-60 overflow-y-auto bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg shadow-lg">
                        @foreach ($drugResults as $res)
                        <button type="button" wire:click="selectDrug({{ $res['id'] }})" wire:key="drug-item-option-{{ $res['id'] }}-{{ $loop->index }}" class="w-full text-left px-4 py-2.5 hover:bg-zinc-50 dark:hover:bg-zinc-800 border-b border-zinc-100 dark:border-zinc-800/80 text-xs flex justify-between items-center transition-colors">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select wire:model="presc_aturan_pakai" class="block w-full rounded-lg border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-3 py-2 text-sm text-zinc-900 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" {{ !$isEditable ? 'disabled' : '' }}>
                        <option value="">Pilih Aturan Pakai</option>
                        @foreach ($aturanPakais as $ap)
                        <option value="{{ $ap->nama_aturan_pakai }}" wire:key="aturan-opt-{{ $ap->id }}">{{ $ap->nama_aturan_pakai }}</option>
                        @endforeach
                    </select>
                </div>
    ...
    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm">
        ...
        <div class="mb-5">
            ...
            <select wire:model.live="selectedLabTestId" class="block w-full rounded-lg border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-3 py-2 text-sm text-zinc-900 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                ...
                <option value="{{ $item->id }}" wire:key="lab-item-option-{{ $item->id }}">
    ...
    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 pb-3 border-b border-zinc-100 dark:border-zinc-800">
            <div class="flex items-center gap-2">
                ...
                <flux:heading size="lg" class="font-bold">6. Dokumen Medis & Persetujuan</flux:heading>
            </div>
            @if ($isEditable)
            <flux:dropdown>
        ...
        <div class="border border-zinc-200 dark:border-zinc-800 rounded-lg overflow-hidden">
            <flux:table>
                ...
                <flux:table.rows>
                    ...
                    <flux:table.row wire:key="doc-row-{{ $doc['type'] }}-{{ $doc['id'] }}">
