<x-master.layout>
    <x-slot:heading>Master Aturan Pakai</x-slot:heading>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="flex items-center gap-4">
                <flux:input wire:model.live.debounce.300ms="search" placeholder="Cari aturan pakai..." icon="magnifying-glass" class="w-full" />
        ...
        <div>
            <div class="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-5 shadow-sm space-y-5">
                <div>
                    <flux:heading size="lg">{{ $selectedId ? 'Edit Aturan Pakai' : 'Tambah Aturan Pakai' }}</flux:heading>
                    <flux:subheading>Tuliskan instruksi pemakaian obat di bawah.</flux:subheading>
                </div>

                <form wire:submit.prevent="save" class="space-y-4">
                    <flux:input wire:model="nama_aturan_pakai" label="Aturan Pakai" required placeholder="Contoh: 3 x 1 Tablet Sehari (Sesudah Makan)" />
